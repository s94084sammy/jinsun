...
def _jinsun_intercept_inbound(text: str):
    """模型之前的硬閘門。命中就擋，不把進線送進 Hermes。"""
    try:
        gate_dir = "/opt/jinsun/mcp"
        if gate_dir not in sys.path:
            sys.path.insert(0, gate_dir)
        from jinsun_mcp import intercept_inbound

        return intercept_inbound(text or "", note="line-adapter")
    except Exception as exc:
        logger.warning("LINE: jinsun inbound gate failed: %s", exc)
        return None
...
LINE_MAX_MESSAGES_PER_CALL = 1  # 金孫一次一則；官方上限 5，實際值看 LINE_MAX_MESSAGES_PER_CALL
...
# 模型想太久時，免費回覆權杖大約 50 到 60 秒會過期。過期後若改走 Push 會扣月額。
# 超過門檻先用免費權杖留「看回覆」鍵，點了再用新的免費權杖送完整那一則。
DEFAULT_SLOW_RESPONSE_THRESHOLD = 40.0  # seconds; 0 would force Push — 金孫不准
DEFAULT_PENDING_REPLY_TEXT = "金孫還在想。好了請按下面看回覆。"
DEFAULT_BUTTON_LABEL = "看回覆"
DEFAULT_DELIVERED_TEXT = "這則已經回過了。"
DEFAULT_INTERRUPTED_TEXT = "這次沒做完，請再說一次。"
...
def build_jinsun_confirm_message(question: str, left: str, right: str) -> Dict[str, Any]:
    """金孫：LINE Confirm 模板，一次兩個大鍵（message action）。

    長輩點了會把鍵上的字當一般訊息送回來，不走任意指令。
    Confirm 限制：text ≤ 240 字，label ≤ 20 字，altText ≤ 400 字。
    """
    q = (question or "請選一個").strip() or "請選一個"
    a = (left or "好").strip() or "好"
    b = (right or "先不要").strip() or "先不要"
    truncated = q if len(q) <= 240 else q[:237] + "..."
    alt = q if len(q) <= 400 else q[:397] + "..."
    return {
        "type": "template",
        "altText": alt,
        "template": {
            ...
            "text": truncated,
            "actions": [
                {"type": "message", "label": a[:20], "text": a[:300]},
                {"type": "message", "label": b[:20], "text": b[:300]},
            ],
        },
    }


_JINSUN_CONFIRM_RE = re.compile(
    r"【金孫兩個鍵】\s*題目[：:]\s*(.+?)\s*左[：:]\s*(.+?)\s*右[：:]\s*(.+?)\s*【結束】",
    re.DOTALL,
)


def parse_jinsun_confirm(content: str) -> Optional[Tuple[str, str, str, str]]:
    """Parse 金孫 two-button marker. Returns (question, left, right, leftover)."""
    if not content or "【金孫兩個鍵】" not in content:
        return None
    m = _JINSUN_CONFIRM_RE.search(content)
    if not m:
        return None
    question, left, right = (m.group(1).strip(), m.group(2).strip(), m.group(3).strip())
    leftover = (_JINSUN_CONFIRM_RE.sub("", content) or "").strip()
    if not question or not left or not right:
        return None
...
class LineAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        # 金孫關掉推播時，慢回覆鍵必須留著。門檻設 0 會在權杖過期後改走 Push。
        if self.slow_response_threshold <= 0 and not allow_push():
            self.slow_response_threshold = DEFAULT_SLOW_RESPONSE_THRESHOLD
    ...
    async def _handle_message_event(self, event: Dict[str, Any]) -> None:
        ...
        # 金孫硬閘門：文字（含語音轉出後若已寫進 text、圖片說明）在進模型前擋轉帳／驗證碼。
        scan_text = text or ""
        hit = _jinsun_intercept_inbound(scan_text)
        if hit:
            logger.info("LINE: jinsun inbound gate blocked reason=%s", hit.get("reason"))
            await self._send_text_chunks(chat_id, hit["reply"], force_push=False)
            return
    ...
    async def _handle_postback_event(self, event: Dict[str, Any]) -> None:
        ...
        if entry.state is State.READY:
            ...
            try:
                ...
                logger.warning("LINE: postback reply failed (%s)", exc)
                if allow_push():
                    try:
                    ...
                    logger.info("LINE: skip metered push after postback reply failure")
    ...
    async def send(
        ...
        # 閘道忙碌回條不送到 LINE：會用掉免費回覆權杖，後面真正的答案就只能走推播。
        if swallow_system_ack(content) or _is_system_bypass(content):
            logger.info("LINE: swallow system ack to keep the free reply token")
    ...
    async def _reply_or_optional_push(
        self,
        chat_id: str,
        ...
        *,
        ...
        """Prefer the free reply token. Push only when explicitly allowed."""
        if not self._client:
            return SendResult(success=False, error="LINE adapter not connected")
        if not messages:
            return SendResult(success=True, message_id=None)
        ...
        if used_reply and not force_push:
            try:
                ...
                logger.info("LINE: reply token rejected (%s)", exc)

        if force_push or allow_push():
            try:
                await self._client.push(chat_id, messages)
                return SendResult(success=True, message_id=None)
            except Exception as exc:
        ...
        logger.info("LINE: skip metered push to stay in free quota")
        return SendResult(success=False, error="LINE push disabled")
    ...
    async def _send_messages(
        ...
        """Send already-built message objects. 金孫一次一則，不拆後續推播批次。"""
        if not messages:
            return SendResult(success=True, message_id=None)
        cap = max_messages_per_call()
        first_batch = messages[:cap]
        if len(messages) > cap:
            logger.info(
                "LINE: dropping %s extra bubbles to keep one-call send",
                len(messages) - cap,
            )
        return await self._reply_or_optional_push(
            chat_id, first_batch, force_push=False
        )
...
async def _standalone_send(
    ...
    *,
    ...
    if not allow_cron_push():
        return {"error": "LINE standalone send: cron push disabled to stay in free quota"}
